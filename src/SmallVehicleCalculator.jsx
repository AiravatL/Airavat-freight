...
const SmallVehicleCalculator = () => {
  ...
  const Card = ({ children, className = "" }) => (
    <div className={`bg-white p-6 rounded-2xl border border-gray-200 shadow-sm ${className}`}>{children}</div>
  );

  // Simple input field
  const InputField = ({ label, value, onChange, suffix, prefix }) => (
    <div>
      <label className="block text-xs font-medium text-gray-500 mb-1.5">{label}</label>
      <div className="flex items-center border border-gray-200 rounded-xl px-4 py-2.5 bg-white">
        {prefix && <span className="text-sm text-gray-700 mr-2">{prefix}</span>}
        <input
          type="number"
          value={value}
          onChange={onChange}
  ...
  const SelectField = ({ label, value, onChange, options }) => (
    <div>
      ...
      <select
        value={value}
        onChange={onChange}
        className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-800 bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/30"
      >
        {options.map((opt) => (
  ...
  // Stat row for breakdown
  const StatRow = ({ label, value, highlight = false }) => (
    <div className="flex justify-between py-1.5">
      <span className="text-sm text-gray-600">{label}</span>
      <span className={`text-sm font-medium ${highlight ? "text-purple-600" : "text-gray-900"}`}>{value}</span>
    </div>
  );
...
export default SmallVehicleCalculator;
