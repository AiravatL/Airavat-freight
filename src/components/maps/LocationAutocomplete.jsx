...
const LocationAutocomplete = ({
    ...
    // NOTE: We intentionally only depend on `value` (not `query`) to prevent race conditions
    // during typing. This effect should ONLY run when the external value changes from parent,
    // not when the internal query changes from user typing.
    const lastExternalValueRef = useRef(value);
    useEffect(() => {
        // Only sync if the external value changed from parent (not from our own onChange)
    ...
    }, [value, setQuery]); // Removed `query` from dependencies to prevent race condition
